...
export interface TechFilterPanelProps extends TechCatalogView {
  searchPlaceholder?: string;
}

// Shared technology filter panel presentation (task 1.3): search box,
// category tabs with tooltip/expand, AND/OR toggle, per-technology
// icon/tags/parent labels/count/selected-excluded styling. Extracted from
// JobTechFilterPanel to be fully props-driven, with no direct store access,
// so it can be reused by both the job and company pages (task 7.2, 3.2).
export function TechFilterPanel({
  techs,
  visibleTabs,
  selectedTab,
  setSelectedTab,
...
}: TechFilterPanelProps) {
  // Lookup from tech id to its full catalog record, so parent references can
  // be rendered by their resolved label instead of the raw id. Built once per
  // techs change: O(n) to build, O(1) to query.
