...
const groupedKeywordGroups = computed(() => {
  ...
  return [...buckets.entries()]
    .sort(
      ([a], [b]) =>
        (CATEGORY_PRIORITY[a] ?? 6) -
        (CATEGORY_PRIORITY[b] ?? 6),
    )
    .map(([cat, items]) => {
...
const link = computed(
  () => new URL(props.company.company_link).host,
);
</script>
  ...
  >
    ...
    <div class="flex items-start justify-between gap-2">
      <div class="min-w-0 flex-1">
        <h2
          class="truncate text-[22px] leading-tight font-black tracking-[-0.02em] text-[#001f2a]"
        ...
        <span
          ...
          class="mt-1 block text-[11px] font-bold tracking-wider text-[#434653]"
        >
    ...
    >
      <div
        v-for="group in groupedKeywordGroups"
        :key="group.label"
      >
        <div
          class="mb-1.5 text-[10px] font-bold tracking-[0.15em] text-[#434653]"
        >
          {{ group.label }}
        </div>
        <div class="flex flex-wrap gap-1">
          ...
          <span
            ...
            class="px-1 py-0.5 text-xs font-bold text-[#434653]/60"
    ...
    >
      <span
        class="flex items-center gap-1 text-xs font-bold text-[#003d92] opacity-0 transition-opacity group-hover:opacity-100"
      >
        查看職缺 →
      </span>
      <a
