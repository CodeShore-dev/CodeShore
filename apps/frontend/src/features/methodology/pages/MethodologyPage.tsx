  ...
  const renderBlock = (block: MethodologyBlock, index: number) => {
    if (block.kind === 'paragraph') {
      return (
        <p
          key={index}
          className="mb-4 text-sm leading-relaxed text-[#1f2330]"
        >
          {block.text}
        </p>
      );
    ...
    if (block.kind === 'list') {
      return (
        <ul
          key={index}
          className="mb-4 list-disc space-y-2 pl-5 text-sm leading-relaxed text-[#1f2330]"
        >
          ...
          <thead>
            <tr>
              {block.headers.map((header, h) => (
                <th
                  key={h}
                  className="border-b border-[#d4d7e0] py-2 pr-4 align-top font-bold text-[#434653]"
                >
                  {header}
                </th>
              ))}
            ...
            {block.rows.map((row, r) => (
              <tr key={r}>
                {row.map((cell, c) => (
                  <td
                    key={c}
                    className="border-b border-[#eceef3] py-2 pr-4 align-top leading-relaxed"
                  >
                    {cell}
                  </td>
                ))}
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-10">
        ...
        <section id="source-sql" className="mb-12 scroll-mt-20">
          ...
          {sqlObjects.map(obj => (
            ...
            >
              <h3 className="font-mono text-sm font-bold break-all text-[#001f2a]">
                {obj.name}
              </h3>
              <p className="mb-2 text-xs text-[#434653]">
                用於：{obj.usedBy.join('、')}
              </p>
              {sqlMap[obj.name] ? (
