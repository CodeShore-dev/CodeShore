 ...
 * 正規化、批次 upsert、列表頁進度與多種執行模式、429 限流退讓、連續無新職缺跳過來源），
 * 僅職缺描述方法與架構。
 *
  ...
  nodes: [
    ...
    },
    {
      ...
      detail: {
        role: '取得本頁職缺與分頁資訊',
        usage:
          '傳統作法：開啟列表頁→等待列表頁渲染→爬取資料；本專案改用監聽 XHR/Fetch 回應，直接攔截列表 API 取得那些即將渲染在畫面上的資料，並解析<strong>頁面概況</strong>（目前頁／總頁數／總筆數）。最多重試 10 次、每次逾時 30 秒，重試時重載頁面，失敗不中斷整體；但若攔截到的回應是 429（被限流），視為需要退讓，不進入上述重試迴圈，記錄後直接優雅停止整個爬蟲，且不更動任何頁面在<strong>職缺來源表</strong>的進度狀態，讓下次續抓自然接手。取得的本頁職缺接著交給「過濾既有職缺」。',
      },
    },
    ...
    },
    {
      ...
      detail: {
        role: '詳細任務 ＋ 下一頁任務（遞迴）',
        usage:
          '每筆新職缺以高優先度建立<strong>抓取詳細</strong>任務，同時以低優先度建立下一頁的<strong>抓取列表</strong>任務，形成自我遞迴直到末頁。第 1 頁時利用<strong>頁面概況</strong>的資訊在<strong>職缺來源表</strong>預建好全部列表頁的待完成紀錄、之後每頁完成即更新狀態，供斷點續抓；「完成」的認定要等本頁所有<strong>抓取詳細</strong>任務都處理完（不論成功或失敗）才算數，避免中斷發生在標記完成與詳細頁真正抓完之間而漏抓。同一個來源若連續 5 頁都沒有新職缺，即放棄該來源剩餘頁面、直接換下一個來源——但「重新開始」重新從第 1 頁爬時，<strong>職缺來源表</strong>會先給一個已知深度下限（見該節點說明），在走到那個深度之前，就算頁面看起來沒有新職缺，也不計入這個放棄判斷，避免把上次還沒真正抓過、可能仍有新職缺的更深分頁誤判為已經抓到底而跳過。',
      },
    },
    {
      id: 'list-next',
    ...
    },
    {
      ...
      detail: {
        role: '擷取職缺欄位',
        usage:
          '等待詳細頁就緒後，在瀏覽器內執行擷取標題、職缺描述、薪資、地點、公司等欄位（職缺描述為空視為職缺已下架，標記 closed）。若詳細頁的回應是 429（被限流），同樣優雅停止整個爬蟲，且刻意不把這個任務算進所屬列表頁的完成計數，讓那一頁維持未完成、這筆職缺隨下次續抓一起補回。擷取到的原始欄位接著送入「正規化」。',
      },
    },
    {
      id: 'normalizer',
    ...
    },
    {
      ...
      detail: {
        role: '續抓機制・列表頁進度',
        usage:
          '續抓的核心：以 status（pending／completed／failed）記錄每個列表頁的處理進度。「重新開始」清空本表之前，會先讀出每個來源目前追蹤到的最大分頁編號，交給「建立任務（遞迴）」當作連續無新職缺放棄判斷的下限——避免清表重爬時，把上次還沒真正抓完的深度誤判為已經抓到底而跳過。接著才真正清空本表，並把每個來源的第 1 頁預先登記為 pending，再開始抓——即使爬蟲在第 1 頁完成前就中斷，這個來源仍有紀錄可續抓，不會因為從未寫入而被整個跳過。第 1 頁完成後再依<strong>頁面概況</strong>一次預建好其餘列表頁的 pending 紀錄、每頁完成即標 completed。下次啟動的續抓模式會撈出仍為 pending 的頁、重新交給「攔截列表 API」接續處理，不重跑已完成的頁，達成斷點恢復。',
      },
    },
  ...
  views: {
    flow: {
      ...
      edges: [
        ...
        // 防誤放棄機制：「重新開始」清空本表前，先讀出已知深度回饋給放棄判斷當下限
        { from: 'db-job-source', to: 'list-enqueue', label: '已知深度下限（重新開始）' },
      ],
    },
    recrawl: {
      id: 'recrawl',
