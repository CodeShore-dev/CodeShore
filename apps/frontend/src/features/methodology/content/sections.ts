    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          ['job', 'ix_job_min_salary_max_salary_updated_at (min_salary DESC, max_salary DESC, updated_at DESC)', '欄位順序刻意對齊 mv_job 定義裡的 ORDER BY，讓 REFRESH MATERIALIZED VIEW CONCURRENTLY 重建彙總結果時可直接依索引順序輸出，省去額外排序。'],
          ...
          ['mv_job', 'ix_mv_job_updated_at (updated_at DESC)', '依更新時間排序，對應預設排序／「最新職缺」。'],
