...
export const methodologySections: readonly MethodologySection[] = [
  ...
  },
  {
    ...
    blocks: [
      {
        kind: 'paragraph',
        text: '本站以 Google Analytics 4（GA4）記錄匿名流量，實作與設定方式如下，供檢視本站如何處理追蹤與同意狀態。',
      },
      {
        kind: 'paragraph',
        text: '如何設定：',
      },
      {
        ...
        rows: [
          [
            '追蹤元件',
            'apps/frontend/src/app/Analytics.tsx：只在有設定 measurement ID、且非本機開發模式時才啟用；掛載時動態載入 gtag.js 並設定 measurement ID，之後每次 react-router 路由切換（含首次進站）都手動回報一次 page_view，取代 gtag 內建、無法感知 SPA 換頁的自動 page_view。',
          ],
          [
            '環境變數',
            '前端以 VITE_GA_MEASUREMENT_ID（apps/frontend/src/config/env.ts 統一讀取 import.meta.env）取得 measurement ID；未設定時此值為空字串，Analytics 元件即維持停用，本機開發與尚未設定的環境都不會誤發送資料。',
          ],
          [
            '密鑰存放',
            'measurement ID 以 secret 形式保存，不寫死在程式碼或版本控制中：GCP 部署用 GCP Secret Manager、Azure／AWS 部署（GitHub Actions）用 repo 層級的 Actions secret，兩邊皆存放同一個值。',
          ],
          [
            '建置期注入',
            '各雲 CI 設定（cloudbuild.yaml／deploy-azure.yml 等）都在 Vite build 之前，把對應密鑰寫入 apps/frontend/.env 的 VITE_GA_MEASUREMENT_ID，讓這個值在建置當下就被打包進前端靜態資產——這是 Vite 專案處理任何 VITE_* 建置期設定值的共同模式，並非 GA4 專屬。',
          ],
        ],
      },
      {
        kind: 'paragraph',
        text: '導入時發現的問題：gtag.js 預設同意狀態為「拒絕」，且失敗不會有任何錯誤訊息。',
      },
      {
        kind: 'table',
        headers: ['問題', '如何發現', '修法'],
        rows: [
          [
            'gtag.js 已正確載入、內部狀態也顯示設定指令有被處理，但瀏覽器從未寫入 GA4 用來識別使用者的 cookie，也從未真正送出量測請求——而且瀏覽器主控台沒有任何錯誤或警告。本站沒有 cookie 同意橫幅，程式碼裡也從未呼叫 gtag 的 consent 設定；gtag.js 在沒有收到明確同意信號時，預設會把 analytics_storage 視為「拒絕」，因而安靜地略過每一筆資料，不留下任何痕跡。',
