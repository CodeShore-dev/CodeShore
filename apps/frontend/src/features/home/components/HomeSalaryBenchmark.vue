...
const benchmarkDescriptions: Record<string, Record<string, string>> = {
  month: {
    均標: '一半的職缺低於這條線。剛畢業 / 1-3 年大概看到這附近。',
    高標: '前 25%。要 5+ 年、外商、或挑得到 stack。',
    頂標: '頂尖 10%。Staff+、FAANG TW、或拿股票的。',
  },
  year: {
    均標: '一半的年薪職缺。多半含獎金與股票。',
    高標: '前 25% 年薪。外商比例變高。',
    頂標: '頂尖 10%。基本上就是 Senior+ 外商了。',
  },
};
...
<template>
  <section class="mt-10">
    ...
    <div v-if="loading" class="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <div v-for="i in 3" :key="i" class="h-44 animate-pulse rounded-xl bg-white shadow-[0_24px_40px_rgba(0,31,42,0.06)]" />
    </div>
